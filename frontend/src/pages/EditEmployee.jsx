...
export default function EditEmployee() {
  ...
  useEffect(() => {
    async function loadData() {
      const roles = await taskService.getMasterRoles() || [];
      setMasterRoles(roles);
      setMasterLocations(await taskService.getMasterLocations() || []);

      const employee = await taskService.getEmployeeById(id);
      if (employee) {
        setName(employee.name || employee.displayName || employee.DisplayName || "");
        setEmail(employee.email || "");
        setPassword(employee.password || "");
        
        let initialRole = employee.role || "";
        if (!initialRole && employee.roleId) {
          const matchedRole = roles.find(r => r.id === employee.roleId || r.roleId === employee.roleId);
          if (matchedRole) initialRole = matchedRole.name;
        }
        setRole(initialRole);
        
        setPlace(employee.location || employee.place || "");
  ...
  const handleEditEmployee = async (e) => {
    ...
    try {
      await taskService.updateEmployee(id, updateData);
