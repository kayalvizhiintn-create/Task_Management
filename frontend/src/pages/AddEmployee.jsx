...
export default function AddEmployee() {
  ...
  useEffect(() => {
    async function loadData() {
      setMasterRoles(await taskService.getMasterRoles() || []);
        ...
        <form onSubmit={handleAddEmployee} className="space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
            ...
            <div className="space-y-2">
              ...
              <input type="text" required value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-5 py-3.5 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 text-slate-900 dark:text-white font-semibold transition-all" />
            </div>
