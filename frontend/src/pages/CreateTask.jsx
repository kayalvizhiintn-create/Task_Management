...
export default function CreateTask() {
  ...
  const [status, setStatus] = useState("Task Create");
  ...
  useEffect(() => {
    const loadDependencies = async () => {
      try {
        const [
          empsRaw, depts, prios, projs, tasks, zns, stats, allT
        ] = await Promise.all([
          taskService.fetchEmployeesAsync(),
        ...
        if (tasks && tasks.length > 0 && !name) setName(tasks[0].name);
  ...
  const handleSubmit = async (e) => {
    ...
    if (proj) {
      ...
      try {
        const shortCode = projectName.substring(0, 3).toUpperCase();
        const res = await apiClient.post('/api/v1/project/create-project', {
          ProjectName: projectName,
          ProjectShortCode: shortCode
        });
        if (res.data && res.data.isSuccess) {
          finalProjectId = res.data.data.projectId;
        } else {
          alert("Failed to create new project: " + (res.data?.message || ""));
          return;
        }
      } catch (err) {
    ...
    const zn = zones.find(z => z.name === zone);
    if (zn) formData.append("ZoneId", zn.id);
    else {
    ...
    if (assignedBy) formData.append("AssignedByBioId", assignedBy);
    if (assignTo) formData.append("AssignedToBioId", assignTo);

    let pri = priorities.find(p => p.name.toLowerCase() === priority.toLowerCase());
    if (!pri && priority) {
      await taskService.addPriority(priority);
      const updatedPriors = await taskService.getPriorities();
      pri = updatedPriors.find(p => p.name.toLowerCase() === priority.toLowerCase());
    }
    if (pri) formData.append("PriorityId", pri.id);

    let stat = statuses.find(s => s.name.toLowerCase() === status.toLowerCase());
    if (!stat && status) {
      await taskService.addStatus(status);
      const updatedStats = await taskService.getStatuses();
      stat = updatedStats.find(s => s.name.toLowerCase() === status.toLowerCase());
    }
    if (stat) formData.append("StatusId", stat.id);

    if (startDate) formData.append("StartDate", startDate);
    if (dueDate) formData.append("EndDate", dueDate);
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder="Type or select a task name..."
                    required
                  />
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <select
                    ...
                    className="w-full px-5 py-3.5 bg-slate-50/50 border border-slate-200 rounded-2xl text-sm font-bold focus:outline-none focus:ring-4 focus:ring-primary/10 focus:border-primary text-slate-700 appearance-none shadow-inner cursor-pointer"
                  >
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    <option value="Branding Identity & Digital Presence">Branding Identity & Digital Presence</option>
                    <option value="Business Development & Strategic Partnerships">Business Development & Strategic Partnerships</option>
                    <option value="Cloud & Infrastructure Management">Cloud & Infrastructure Management</option>
                    <option value="Corporate Compliance & Legal Setup">Corporate Compliance & Legal Setup</option>
                    <option value="Events, Customer Engagement & Corporate Networking">Events, Customer Engagement & Corporate Networking</option>
                    <option value="Finance, Banking & Accounts">Finance, Banking & Accounts</option>
                    <option value="Hardware & IT Infrastructure">Hardware & IT Infrastructure</option>
                    <option value="HR & Oraganizational Development">HR & Oraganizational Development</option>
                    <option value="Marketing, Branding & Corporate Communication">Marketing, Branding & Corporate Communication</option>
                    <option value="Office Infrastructure & Administration">Office Infrastructure & Administration</option>
                    <option value="Software Licensing & Development Tools">Software Licensing & Development Tools</option>
                    <option value="Website & Media Development">Website & Media Development</option>
                  </select>
                  <div className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-slate-400">
                    <LayoutGrid size={16} />
                  </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-8">
              ...
              <div className="space-y-2">
                ...
                <select
                  ...
                  disabled
                  className="w-full px-5 py-3.5 bg-slate-50/50 border border-slate-200 rounded-2xl text-sm font-bold focus:outline-none focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 text-slate-700 shadow-inner opacity-70 cursor-not-allowed"
                >
