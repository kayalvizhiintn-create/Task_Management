...
export default function EmployeeProfileView() {
  ...
  useEffect(() => {
    async function loadData() {
      const found = await taskService.getEmployeeById(id);
      if (found) {
        setEmployee({ ...found, name: found.name || found.displayName || found.DisplayName || "Unknown" });
      }
  ...
  const handleDeleteEmployee = async () => {
    if (window.confirm(`Are you sure you want to delete ${employee.name}?`)) {
      await taskService.deleteEmployee(employee.id);
      navigate("/directory");
    }
  };
