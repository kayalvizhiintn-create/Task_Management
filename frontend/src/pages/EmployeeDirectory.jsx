...
export default function EmployeeDirectory() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const usersRes = await taskService.getAllUsers?.();
        const apiData = usersRes?.data || usersRes || [];

        if (apiData && apiData.length > 0) {
          // Map API user format to employee format
          const formatted = apiData.map(u => {
            const empName = u.name || u.userName || u.user_name || u.UserName || u.displayName || u.DisplayName || u.email || "Unknown";
            return {
              id: u._id || u.id || u.userId,
              name: empName,
              role: u.role || "Team Member",
              place: u.location || "Remote",
              email: u.email || "",
              avatar: u.avatar || u.img || `https://ui-avatars.com/api/?name=${encodeURIComponent(empName.charAt(0))}&background=random`
            };
          });
          setEmployees(formatted);
        } else {
          const mockEmps = await taskService.getEmployees() || [];
          setEmployees(mockEmps.map(u => {
            const empName = u.name || u.userName || "Unknown";
            return {
              ...u,
              name: empName,
              avatar: u.avatar || u.img || `https://ui-avatars.com/api/?name=${encodeURIComponent(empName.charAt(0))}&background=random`
    ...
    fetchData();
    taskService.getMasterRoles().then(roles => setMasterRoles(roles || []));
    taskService.getMasterLocations().then(locs => setMasterLocations(locs || []));
  }, []);
  ...
  const handleDeleteEmployee = async (id, empName) => {
    if (window.confirm(`Are you sure you want to delete ${empName}?`)) {
      ...
      setEmployees(await taskService.getEmployees() || []);
