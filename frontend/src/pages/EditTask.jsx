...
export default function EditTask() {
  ...
  const [toastMessage, setToastMessage] = useState("Task modified successfully!");
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      setEmployees(emps || []);
      setTasksMaster(tasksMasterList || []);
      setProjects(projs || []);
      ...
      let task = (allT || []).find(t => t.id && t.id.toString() === id.toString());

      // If not found in mapped allT, try the direct API but ensure we look at the data property
      if (!task) {
        ...
        if (rawTask) {
          task = {
            ...
            category: rawTask.departmentName || rawTask.category || rawTask.department,
            projectName: rawTask.projectName || rawTask.ProjectName,
            ...
            department: departments.find(d => d.id === (rawTask.deptId || rawTask.DeptId))?.name || ""
          };
        }
      }
  ...
  const handleSaveEdit = async (e) => {
    ...
    const proj = projects.find(p => p.name === editFormData.projectName);
    const dept = departments.find(d => d.name === editFormData.department);
    const zn = zones.find(z => z.name === editFormData.zone);
    let pri = priorities.find(p => p.name.toLowerCase() === editFormData.priority.toLowerCase());
    if (!pri && editFormData.priority) {
      await taskService.addPriority(editFormData.priority);
      const updatedPriors = await taskService.getPriorities();
      pri = updatedPriors.find(p => p.name.toLowerCase() === editFormData.priority.toLowerCase());
    }

    let stat = statuses.find(s => s.name.toLowerCase() === editFormData.status.toLowerCase());
    if (!stat && editFormData.status) {
      await taskService.addStatus(editFormData.status);
      const updatedStats = await taskService.getStatuses();
      stat = updatedStats.find(s => s.name.toLowerCase() === editFormData.status.toLowerCase());
    }
    ...
    setToastMessage("Task modified successfully!");
    setShowToast(true);
    setTimeout(() => {
      setShowToast(false);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
            ...
            <div className="space-y-2">
              ...
              >
                ...
                {editFormData.status && !statuses.find(s => s.name.toLowerCase() === editFormData.status.toLowerCase()) && (
