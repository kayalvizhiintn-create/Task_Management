...
const navItems = [
    {
        ...
        subItems: [
            { path: "/", icon: <Activity size={18} />, label: "Main Dashboard" },
...
export default function Sidebar({ onClose }) {
    ...
    return (
        <aside className="w-64 bg-slate-950 border-r border-slate-800 flex flex-col h-full shadow-2xl transition-colors duration-300 relative overflow-hidden z-20 print:hidden">
            ...
            <div className="p-4 border-t border-slate-800/50 relative z-10 bg-slate-950/80 backdrop-blur-sm flex flex-col gap-3">
                {currentUser && (
                    <div className="flex items-center gap-3 px-3 py-2.5 bg-slate-900/80 rounded-2xl border border-slate-800/80 shadow-inner">
                        <img 
                            src={currentUser.avatar || "src/assets/logo.jpg"} 
                            alt={currentUser.name} 
                            className="w-10 h-10 rounded-xl object-cover ring-2 ring-indigo-500/20"
                        />
                        <div className="flex-1 min-w-0">
                            <p className="text-sm font-bold text-white truncate leading-tight">{currentUser.name || currentUser.displayName || currentUser.userName || "Kayal"}</p>
                            <p className="text-[10px] uppercase tracking-wider text-slate-400 truncate mt-0.5 font-semibold">{currentUser.role || "User"}</p>
                        </div>
                    </div>
                )}
                ...
                <button
                    ...
                    className="flex items-center justify-center gap-2 px-4 py-3 w-full rounded-2xl text-slate-400 hover:bg-rose-500/10 hover:text-rose-400 hover:border-rose-500/20 border border-slate-800/50 transition-all duration-300 group font-bold"
                >
