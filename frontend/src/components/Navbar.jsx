...
export default function Navbar({ onSearchChange, onMenuClick }) {
  ...
  useEffect(() => {
    ...
    const loadActivities = async () => {
      try {
        const activities = await taskService.getActivities();
          ...
          <div className="relative" ref={profileRef}>
            ...
            >
              ...
              <div className="text-left hidden md:block">
                <p className="text-sm font-bold text-slate-800 tracking-tight leading-tight">{currentUser.name || currentUser.displayName || currentUser.userName || "Unknown User"}</p>
                <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider text-[10px] mt-0.5">{currentUser.role || "User"}</p>
              </div>
            ...
            {showProfileMenu && (
              <div className="absolute right-0 mt-3 w-56 bg-white border border-slate-200 rounded-2xl shadow-floating overflow-hidden z-50 animate-slide-up">
                <div className="p-4 border-b border-slate-100 bg-slate-50/50">
                  ...
                  <p className="text-sm font-bold text-slate-800 truncate mt-0.5">{currentUser.email || currentUser.name || currentUser.displayName || currentUser.userName || "Unknown User"}</p>
                </div>
