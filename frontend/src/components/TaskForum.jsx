...
export default function TaskForum({ currentUser, employees }) {
  // Mock initial comments for the UI demonstration
  const [comments, setComments] = useState([
    {
      id: 1,
      userId: currentUser?.id || 1,
      userName: currentUser?.name || "John Doe",
      userAvatar: currentUser?.avatar || "https://i.pravatar.cc/150?u=1",
      text: "I have started working on the initial layout. @jane please review when you have time.",
      attachment: null,
      ...
      replies: [
        {
          ...
          text: "Looks good to me! I'll check the responsiveness later.",
  ...
  const formatTime = (isoString) => {
    ...
    const now = new Date();
    const diff = Math.floor((now - date) / 60000); // mins
  ...
  // Helper to parse mentions in text to styled spans
  ...
  const handleSubmit = (e) => {
    ...
    if (editingId) {
      ...
      if (editingId.type === 'comment') {
        const cIdx = updatedComments.findIndex(c => c.id === editingId.commentId);
        if (cIdx > -1) {
          updatedComments[cIdx].text = inputText;
          if (attachment) updatedComments[cIdx].attachment = attachment;
        }
      } else if (editingId.type === 'reply') {
        const cIdx = updatedComments.findIndex(c => c.id === editingId.commentId);
        if (cIdx > -1) {
          const rIdx = updatedComments[cIdx].replies.findIndex(r => r.id === editingId.replyId);
          if (rIdx > -1) {
            updatedComments[cIdx].replies[rIdx].text = inputText;
          }
        }
      }
      ...
      // Handle Reply
      const updatedComments = [...comments];
      const cIdx = updatedComments.findIndex(c => c.id === replyingTo.commentId);
      if (cIdx > -1) {
        updatedComments[cIdx].replies.push({
          id: Date.now(),
          userId: currentUser?.id || 1,
          userName: currentUser?.name || "Current User",
          userAvatar: currentUser?.avatar || "https://i.pravatar.cc/150?u=currentUser",
          text: inputText,
  ...
  const handleDelete = (type, commentId, replyId = null) => {
    ...
    if (type === 'comment') {
      ...
      const cIdx = updated.findIndex(c => c.id === commentId);
      if (cIdx > -1) {
        updated[cIdx].replies = updated[cIdx].replies.filter(r => r.id !== replyId);
        setComments(updated);
      }
    }
  };
  ...
  // Very basic mention detection during typing
  useEffect(() => {
                ...
                <div className="flex-1 space-y-1.5">
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      {(currentUser?.id === comment.userId || !currentUser) && (
                        <>
                  ...
                  <div className="text-sm text-slate-700 leading-relaxed font-medium bg-slate-50/70 p-3.5 rounded-2xl rounded-tl-none border border-slate-100">
                  ...
                  {comment.attachment && (
                    <div className="mt-2">
                      {comment.attachment.type === 'image' ? (
                        ...
                        <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 p-2.5 rounded-xl w-max">
                          ...
                          <div className="flex-1 space-y-1">
                            <div className="flex justify-between items-start">
                              ...
                              <div className="flex items-center gap-1 opacity-0 group-hover/reply:opacity-100 transition-opacity">
                                {(currentUser?.id === reply.userId || !currentUser) && (
                                  <>
                            ...
                            <div className="text-sm text-slate-600 font-medium bg-slate-50 p-2.5 rounded-xl rounded-tl-none border border-slate-100">
      ...
      <div className="relative pt-4 border-t border-slate-100">
        ...
        <form onSubmit={handleSubmit} className="relative z-10 flex flex-col gap-2 bg-slate-50 border border-slate-200 rounded-2xl p-2 shadow-sm focus-within:ring-2 focus-within:ring-primary/20 focus-within:border-primary transition-all">
          <textarea
            ...
            className="w-full bg-transparent border-none focus:ring-0 resize-none px-2 py-1 text-sm font-medium text-slate-800 placeholder-slate-400 custom-scrollbar"
            rows="2"
            onKeyDown={(e) => {
          ...
          <div className="flex justify-between items-center px-2 pb-1">
            <div className="flex items-center gap-1">
              ...
              <button 
                ...
                title="Attach File"
                disabled={!!editingId} // Disable attachment changes during edit for simplicity in mock
              >
                <Paperclip size={18} />
              </button>
            </div>
